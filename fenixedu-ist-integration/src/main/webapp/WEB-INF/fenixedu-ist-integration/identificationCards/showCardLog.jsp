		...
		<tbody>
			<c:forEach var="request" items="${requests}">
				<tr>
					...
					<td>
						<a href="#" class="btn btn-default" onclick="displayDetails('srd${request.externalId}'); return false;">
							<spring:message code="label.details" text="Details"/><!-- TODO -->
						</a>
					</td>
				</tr>
				<tr id="srd${request.externalId}" style="background-color:#F0F0F0; display: none;">
					<td colspan="10">
						<div style="background-color:#F0F0F0; height: 400px; overflow: scroll;">
							...
							<div id="srdm${request.externalId}"></div>
							<h4>
								<spring:message code="label.request" text="Request"/>
							</h4>
							<div id="srdr${request.externalId}"></div>
						</div>
					</td>
				</tr>
			</c:forEach>
...
<c:forEach var="request" items="${requests}">
    <script type="text/javascript">
        $(document).ready(function() {
        	var responseAsJson = ${request.responseAsJson};
        	var requestAsJson = ${request.requestAsJson};
            integrationPart('srdm${request.externalId}', responseAsJson);
            integrationPart('srdr${request.externalId}', requestAsJson);
        });
    </script>
</c:forEach>
