...
<section>
    <c:choose>
        <c:when test="${fn:length(paymentMethods) >= 2}">
            <form:form role="form" method="POST" class="form-horizontal" action="${pageContext.request.contextPath}/client-management/manageDefaultPaymentMethods">
                ${csrf.field()}
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-1">
                        <select name="defaultInternalPaymentMethod" id="defaulttInternalPaymentMethod">
                            <c:forEach var="paymentMethod" items="${paymentMethods}">
                                <option value="${paymentMethod.externalId}" <c:if test="${not empty paymentMethod.internalBennu}">selected="selected"</c:if> >${paymentMethod.code} - ${paymentMethod.localizedName}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
