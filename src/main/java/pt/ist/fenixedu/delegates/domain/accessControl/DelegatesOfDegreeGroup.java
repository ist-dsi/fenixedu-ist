...
public class DelegatesOfDegreeGroup extends GroupStrategy {

    private static final long serialVersionUID = -7017075302622415050L;

    @Override
    public Set<User> getMembers(DateTime when) {
        return Bennu.getInstance().getDelegatesSet().stream()
                .filter(d -> d instanceof DegreeDelegate || d instanceof CycleDelegate && d.isActive(when)).map(d -> d.getUser())
    ...
    public boolean isMember(User user, DateTime when) {
        if (user.getDelegatesSet().stream().filter(d -> d instanceof DegreeDelegate || d instanceof CycleDelegate)
                .map(d -> d.isActive(when)).reduce(false, (x, y) -> x || y)) {
            return true;
        }
        return false;
    }
